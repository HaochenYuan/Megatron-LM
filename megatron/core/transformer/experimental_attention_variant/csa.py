    ...
    * ``ratio == 128``: window + 128x compressed, attend to all (compressor built only)
    ...
    def _get_thd_compact_indexer_workspace(
        ...
        *,
        ...
        if capturing:
            # Under full activation recompute the checkpoint forward (no_grad -> inference path)
            # and its recompute (training path) alternate between two geometries
            # (return_softmax differs), so the *last prepared* workspace is not necessarily the
            # one this capture needs. Any warmed-up workspace may be reused: matching is
            # shape-only (no host sync).
            candidates = [workspace] if workspace is not None else []
            candidates += [w for w in self._thd_compact_indexer_workspaces if w is not workspace]
