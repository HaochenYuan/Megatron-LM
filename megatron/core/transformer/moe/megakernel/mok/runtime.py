...
class _MoKAutograd(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if os.environ.get("MCORE_MOK_COPY_GRAD_OUT", "0") == "1":
            # EXPERIMENT: MOK hands back gradients that live in its SymmetricMemory workspace.
            # TE's graphed-callable backward (with _reuse_graph_input_output_buffers) clones
            # grad inputs via cudaMemcpyAsync, which fails with "invalid argument" on that
            # mapping. Materialize the gradients in ordinary allocator memory with a kernel copy.
            d_x = torch.mul(d_x, 1)
            d_router_weights = torch.mul(d_router_weights, 1)
        return (
            None,
            d_x,
            d_router_weights,
